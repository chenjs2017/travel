    ...
    <div class="st-inbox-body">
        ...
        <div class="inbox-list-message">
            ...
            foreach($list_message as $key => $val){
                ...
                $link_detail = remove_query_arg('search', $current_link);
                $link_detail = add_query_arg(array('message_id' => $val['id']), $current_link);

                ?>
                <a href="<?php echo esc_url($link_detail); ?>" class="show-detail <?php echo esc_attr($class); ?>" data-id="<?php echo esc_attr($val['id']); ?>"><?php echo wp_trim_words($val['title'], 20, '...'); ?></a>
