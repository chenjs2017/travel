    ...
    class ST_PDF_Invoices_Controller{
        ...
        function _init_invoice(){
            ...
            if($invoice_enable == 'on'){
                add_action('st_after_header_order_information_table', array($this, '_show_title'), 20);
                add_filter('st_after_body_order_information_table',array($this, '_show_button_download'), 15, 2);
        ...
        function _show_title(){
            echo '<th>';
            echo esc_html__('Invoice',ST_TEXTDOMAIN);
            echo '</th>';
        }
        ...
        function _show_button_download($res,$order_id){
            if(!empty($order_id)){
                $current_url = $_SERVER['REQUEST_URI'];
                $current_url = add_query_arg(array(
                    'st_download' => $order_id
                ), $current_url);
                return '<td class="text-center"><a target="_blank" class="wb-pdf-invoices-button" href="' . esc_url($current_url) . '" ><i class="fa fa-download fa-2x"></i></a></td>';
            }
        ...
        function _handling_invoice_pdf(){
            if(!empty($this->order_id = STInput::get('st_download'))){
                ...
                $order_data = array();
                global $wpdb;
                $sql = "SELECT * FROM {$wpdb->prefix}st_order_item_meta WHERE wc_order_id = ".$this->order_id;
                $rs = $wpdb->get_row($sql);
